...
透过口塞说话支持翻译数字和日语假名。
定时锁和主人锁可以共存。
由于 Telegram 的限制，频道身份用户透过口塞说话需要同时 /gag 个人身份。
...
\`\`\`
@${config.botUsername} 要透过口塞说的话…（点击复制）
\`\`\`
*__常用命令__*
大部分命令可回复消息以选择目标。如无目标或目标无效，则目标默认为自己。
方括号内为可选参数，可以不填。
...
[点击跳转查看完整帮助](https://t.me/${config.botUsername}?start=help)
`,
    gagHelp: `\
*__口塞列表__（效果从弱到强）*
...
`,
    seePM: `为保持群内整洁，请 \
[点击此处跳转查看](https://t.me/${config.botUsername}?start=$command)。
`,
